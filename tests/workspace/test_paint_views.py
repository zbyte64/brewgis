...
class TestMatchBuiltFormView(TestCase):
    ...
    def test_requires_the_parcels_land_development_category(self):
        ...
        self.client.force_login(self.user)
        # 20 du/acre: the rural type (19) is nearer than the urban one (21),
        # and the low-density type (2) is nearer still.
        p1, p2 = self._patch(
            {
                "1": {
                    "id": 1,
    ...
    def test_prefers_the_type_sharing_the_parcels_employment_sector(self):
        """A parcel whose jobs are military takes the military type."""
        ...
        # nearer to the parcel's 0.5 jobs/acre than the military type's 0.3, so
        # only the sector preference can put the military type first
        BuildingTypeFactory(
            workspace=self.workspace,
            name="Parking Lot",
            emp_per_acre=0.4,
            jobs_by_sector={"other_services": 100.0},
        )
        self.client.force_login(self.user)
        p1, p2 = self._patch(
            {
    ...
    def test_falls_back_to_density_without_sector_candidate(self):
        """With no type declaring the parcel's sector, density still decides."""
        self.client.force_login(self.user)
        p1, p2 = self._patch(
            {
                "1": {

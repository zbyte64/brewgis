...
def see_import_tabs(page, tab_list: str) -> None:
    ...
    for tab in expected:
        assert tab in labels, f"Expected tab '{tab}' in import center, got {labels}"
...
def page_title_is(page, title: str) -> None:
    ...
    assert actual == title, f"Expected page title '{title}', got '{actual}'"
...
def breadcrumb_shows(page, crumbs: str) -> None:
    ...
    for crumb in expected:
        assert crumb in path, f"Expected breadcrumb '{crumb}' in path, got {path}"
...
def tab_has_link(page, tab: str, link_text: str) -> None:
    ...
    assert link.is_visible(), f"Expected link '{link_text}' in tab '{tab}'"
...
def tab_has_sub_tabs(page, tab: str, sub_tabs: str) -> None:
    ...
    for sub in expected:
        assert sub in labels, f"Expected sub-tab '{sub}' in tab '{tab}', got {labels}"

...
-- instead of driveable roads. Overture transport data is global — the region
-- is determined by the staging VIEW's bbox variables — so the same SQL
-- serves every region.
...
-- post_statements
  CREATE INDEX IF NOT EXISTS idx_@{region}_path_intersection_points_geometry_@snapshot_hash

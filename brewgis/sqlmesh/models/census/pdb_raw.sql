MODEL (
  ...
  kind VIEW,
  description 'DuckDB staging VIEW of the Census Planning Database block group variables fetched live from the 2024 PDB API via httpfs, one row per block group.',
  column_descriptions (
    gidbg = 'Census block group GEOID (12-digit state+county+tract+block group FIPS), API field 1.',
    ...
    state = 'Two-digit state FIPS code from the API in= clause (field 12).',
    county = 'Three-digit county FIPS code from the API in= clause (field 13).',
    data_year = 'PDB vintage year (2024), the ACS 2018-2022 release the API serves.'
  ),
  gateway duckdb,
  dialect duckdb,

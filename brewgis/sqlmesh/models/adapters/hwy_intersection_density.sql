MODEL (
  ...
  description 'Per-parcel Overture highway interchange density within 1/4 mile of the parcel centroid.',
  column_descriptions (
    apn = 'Assessor parcel number (APN) the density belongs to.',
    highway_intersection_density = 'Motorway/trunk intersection points within 402 m of the centroid (per sq mi).',
...
-- post_statements
  CREATE INDEX IF NOT EXISTS @snapshot_hash('idx_hwy_density_apn_')
  ON @this_model USING btree (apn);
  CREATE INDEX IF NOT EXISTS @snapshot_hash('idx_hwy_density_geometry_')

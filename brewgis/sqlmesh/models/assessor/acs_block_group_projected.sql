MODEL (
  ...
  kind FULL,
  description 'ACS block group demographics reprojected to the region local_srid, one row per block group with homes.',
  column_descriptions (
    ...
    geometry = 'Block group polygon reprojected to the region local_srid for indexed spatial joins.',
...
--
-- Reads ACS block group data from staging, transforms geometry to the region's
-- local_srid, and applies a GiST index on the projected geometry.
--
...
SELECT
    ...
    a.cost_burden_pct,
    ST_Transform(a.geometry, @VAR('local_srid')) AS geometry,
    ST_Envelope(ST_Transform(a.geometry, @VAR('local_srid'))) AS local_envelope,
    GREATEST(@local_area_sqm(ST_Area(ST_Transform(a.geometry, @VAR('local_srid')))), 1e-10) AS bg_area

...
SELECT
  ...
  t1.built_form_key,
  CASE
    WHEN COALESCE(sd.sales_lot_size_acres, 0) < 0.15 THEN 'detsf_sl'
    ELSE 'detsf_ll'
  END AS expected_bft
FROM @this_model t1
...
WHERE (sd.property_type IN ('SFR', 'Single Family Residence') OR sd.property_type LIKE 'Single Family%')
  AND (
    (COALESCE(sd.sales_lot_size_acres, 0) < 0.15 AND t1.built_form_key != 'detsf_sl')
    OR (COALESCE(sd.sales_lot_size_acres, 0) >= 0.15 AND t1.built_form_key != 'detsf_ll')
  );

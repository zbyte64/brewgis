...
-- lot ≥ 0.15 acres → detsf_ll; lot < 0.15 → detsf_sl
--
...
SELECT
  ...
  t2.built_form_key,
  CASE
    WHEN COALESCE(ap.lot_size_acres, 0) < 0.15 THEN 'detsf_sl'
    ELSE 'detsf_ll'
  END AS expected_bft
FROM @this_model t2
JOIN brewgis.assessor.sacog_assessor_parcels ap ON t2.apn = ap.apn
WHERE t2.built_form_key IN ('detsf_sl', 'detsf_ll')
  AND LEFT(COALESCE(ap.landuse, ''), 2) NOT IN ('A2', 'AT')
  AND (
    (COALESCE(ap.lot_size_acres, 0) < 0.15 AND t2.built_form_key != 'detsf_sl')
    OR (COALESCE(ap.lot_size_acres, 0) >= 0.15 AND t2.built_form_key != 'detsf_ll')
  );

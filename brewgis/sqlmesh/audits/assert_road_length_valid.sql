AUDIT (
  name assert_road_length_valid,
  dialect postgres
);

-- Assert that at least 0.1% of parcels have non-zero road total length.
...
WITH stats AS (
    SELECT
        ...
        COUNT(CASE WHEN road_total_length_m > 0 THEN 1 END) AS nonzero_parcels,
        ROUND(
            100.0 * COUNT(CASE WHEN road_total_length_m > 0 THEN 1 END)
            / NULLIF(COUNT(*), 0),
            4
        ) AS nonzero_pct
    FROM @this_model

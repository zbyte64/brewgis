...
-- Tier4 produces detsf_sl/detsf_ll for parcels that fall through all higher
-- tiers (tier0 through tier3). This audit checks that tier4's SL/LL decisions
-- are consistent with the intersection-density sigmoid used by tier0.
--
...
WHERE t4.built_form_key IN ('detsf_sl', 'detsf_ll')
  AND (
    (t4.built_form_key = 'detsf_sl'
        AND 1.0 / (1.0 + EXP(-0.04 * (COALESCE(id.intersection_density, 225.0) - 225.0))) <= 0.5)
    OR
    (t4.built_form_key = 'detsf_ll'
        AND 1.0 / (1.0 + EXP(-0.04 * (COALESCE(id.intersection_density, 225.0) - 225.0))) > 0.5)
  );

...
from brewgis.contrib.footprint.lib.functions import flat_map, any_true, map_to_dict, dual_map_to_dict, map_dict, compact_dict

from footprint.main.mixins.name import Name
from brewgis.contrib.footprint.database.information_schema import InformationSchema
from brewgis.contrib.footprint.feature.feature_class_creator import FeatureClassCreator
from brewgis.contrib.footprint.feature.feature_field_mixin import FeatureFieldMixin
...
from brewgis.contrib.footprint.utils.dynamic_subclassing import create_tables_for_dynamic_classes, drop_tables_for_dynamic_classes, dynamic_model_class
from brewgis.contrib.footprint.utils.model_utils import model_field_paths, limited_api_fields
from brewgis.contrib.footprint.utils.query_parsing import parse_query, related_field_paths
from brewgis.contrib.footprint.utils.utils import parse_schema_and_table, clear_many_cache_on_instance_field, normalize_null, get_property_path, \
    resolve_module_attr
...
def get_or_create_layer_selection_class_for_layer(layer, config_entity=None, no_table_creation=False):
    ...
    dynamic_class = dynamic_model_class(
        ...
        related_class_lookup=dict(
            config_entity='brewgis.contrib.footprint.config.config_entity.ConfigEntity',
            layer='brewgis.contrib.footprint.presentation.layer.layer.Layer'
        ),
        fields=dict(
            features=models.ManyToManyField(feature_class, through=dynamic_through_class, related_name=table_name)
        )

...
10. **`{"success": False, "error": "..."}` return dicts instead of exceptions** — This pattern obfuscates failure locations because the caller must remember to check the return value; an unguarded caller propagates the error to a confusing downstream crash. A bare `raise` / unhandled exception produces a stack trace pointing at the exact line that failed, with the OS or DB error message intact. Only use error-return dicts when the failure is an expected business condition (not a bug), every caller already handles it, and raising would force superfluous try/except at every call site that would be worse.
